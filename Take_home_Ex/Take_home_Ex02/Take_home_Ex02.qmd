...
hexagon_busstop_tidy <- hexagon_busstop %>% 
  st_drop_geometry() %>% 
  select(index, BUS_STOP_N, BUSINESS_COUNT, FOOD_COUNT, RETAIL_COUNT, TRAIN_COUNT, HDB_COUNT, SCHOOL_COUNT)
```
...
The output indicates 2484 observations
```{r}
duplicate <- flow_data %>%
  group_by_all() %>%
  filter(n()>1) %>%
...
# 9. Create Spatial Interaction Model
...
corrplot.mixed(vars.cor,
         lower = "ellipse", 
               ...
               tl.col = "black")
```
# 10. Unconstrained Spatial Interaction Model
...
# 11. Origin (Production) Constrained SIM 
...
# 12. Destination (Attraction) Constrained SIM
