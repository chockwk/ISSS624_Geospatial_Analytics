...
We will be using the following **geospatial** (`busstops`, `subzone`) and **aspatial** (`odbus`, `malls`) datasets.
...
First, we will create hexagonal grids using `st_make_grid()` to cover the geometry of `busstops`.
...
#| code-fold: true
#| code-summary: "Show the code"
mapview(busstops, cex = 3, alpha = 0.5, popup = NULL, col.regions = "darkseagreen")
```
...
highdensityhex = filter(hexagon_grid_sf, grid_id > 8)

tmap_mode("view")
highdensity = tm_shape(highdensityhex)+
  tm_fill(
    col = "grid_id",
    palette = "Greens",
    style = "cont",
...
# Assuming highdensityhex is already an sf object with a geometry column
# and has a 'grid_id' column for fill
# Filter highdensityhex to include only the desired grid_id values
highdensityhex <- highdensityhex[highdensityhex$grid_id > 8, ]
...
# Assuming malls_sf is your spatial object for malls
# and highdensityhex is your spatial object for high-density hexagons
# Set the tmap mode to interactive viewing
tmap_mode("view")

# Create the map
highdensity_malls <- tm_shape(highdensityhex) +
  tm_polygons(
    col = "grid_id",
    palette = "Greens",
    style = "cont",
  ...
  tm_borders(col = "grey40", lwd = 0.7) +
  tm_shape(malls_sf) +
  tm_dots(size = 0.05, col = "salmon", title = "Malls") # Add malls as cyan dots
